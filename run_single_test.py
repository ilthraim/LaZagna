...
def run_one_benchmark(i, blif_file="", verilog_file="", act_file="", original_dir="", width="", height="", channel_width="", type_sb="full", percent_connectivity=0.5, place_algorithm="cube_bb", verilog_benchmarks=False, connection_type="subset", benchmark_top_name="", output_folder_name=""):
...
def main():
    skip_percents = [1]
    # percents_to_test = [round(i * 0.01, 2) for i in range(16, 101)]  # Generate list of values from 0.01 to 1.00
    percents_to_test = [0.5]
    # place_algs = ["per_layer_bb", "cube_bb"]
    place_algs = ["cube_bb"]
    # connection_types = ["subset", "wilton"]
    connection_types = ["subset"]
    type_sbs = ["combined"]
    random_seed_array = [random.randint(0, 100000) for _ in range(0, 100)]

    tasks_start_time = time.time()

    for k in range(len(connection_types)):
        for j in range(len(percents_to_test)):
            if percents_to_test[j] in skip_percents:
                print(f"Skipping {percents_to_test[j]}% connectivity")
                continue
            for i in range(len(place_algs)):
                ...
                connection_type = connection_types[k] # "subset" or "wilton"
                ...
                task_run_folder = setup_flow(original_dir, new_width, new_height, channel_width, type_sb, percent_connectivity=percent_connectivity, place_algorithm=place_algorithm, verilog_benchmarks=verilog_benchmarks, connection_type=connection_type, arch_file=arch_file)
                ...
                with ThreadPoolExecutor() as executor:
                    futures = [
                        executor.submit(run_one_benchmark, i, blif_files[i], verilog_files[i], act_files[i], original_dir, new_width, new_height, channel_width, type_sb, percent_connectivity, place_algorithm, verilog_benchmarks, connection_type, 
                                        top_module_names[os.path.basename(verilog_files[i])] if verilog_benchmarks else [], task_run_folder)
